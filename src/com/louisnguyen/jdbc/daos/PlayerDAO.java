package com.louisnguyen.jdbc.daos;
...
import com.louisnguyen.models.player.Friend;
import com.louisnguyen.models.player.Fusion;
    ...
    public static boolean createNewPlayer(int userId, String name, byte gender, int hair) {
        try {
            ...
            GirlkunDB.executeUpdate("insert into player"
                    ...
                    + "data_black_ball, data_side_task, data_card, bill_data, info_phoban,data_item_time_sieu_cap) "
                    ...
                    itemTime, task, mabuEgg, charms, skills, skillsShortcut, petData, dataBlackBall, dataSideTask, data_card, bill_data, info_phoban, data_item_time_sieu_cap);
        ...
        if (player.iDMark.isLoadedAllDataPlayer()) {
            ...
            try {
                ...
                if (player.isDie()) {
                    ...
                    hp = 1;
                    mp = 1;
                } else if (MapService.gI().isMapDoanhTrai(mapId) || MapService.gI().isMapGiaiCuuMiNuong(mapId) || MapService.gI().isNguHanhSon(mapId) || MapService.gI().isMapBlackBallWar(mapId)
...
//                dataArray.add(player.itemTime.iconMeal);
                ...
                String query = " update player set dhtime = ?, dhtime2 = ?, dhtime3 = ?, dhieu = ?, data_item_time_sieu_cap = ?, head = ?, have_tennis_space_ship = ?,"
                        ...
                        + " skills_shortcut = ?, pointPvp=?, NguHanhSonPoint=?,data_card=?,bill_data =?,info_phoban =? where id = ?";
