...
public class ShopKyGuiService {
    ...
    private List<ItemKyGui> getItemKyGui2(Player pl, byte tab, byte to, byte max) {
        ...
        ShopKyGuiManager.gI().listItem.stream().filter((it) -> (it != null && it.tab == tab && !it.isBuy))
                .forEachOrdered((it) -> {
                    its.add(it);
                });
    ...
    public void StartupItemToTop(Player pl) {
        if (!SubThoiVang(pl, 1)) {
            Service.gI().sendThongBao(pl, "Bạn cần có ít nhất 1 thỏi vàng đưa vật phẩm lên trang đầu");
            return;
        }
    ...
    public List<ItemKyGui> getItemCanKiGui(Player pl) {
        ...
        pl.inventory.itemsBag.stream().filter((it) -> (it.isNotNullItem())).forEachOrdered((it) -> {
            its.add(new ItemKyGui(InventoryServiceNew.gI().getIndexBag(pl, it), it.template.id, (int) pl.id, (byte) 4,
