...
public class ShopServiceNew {
    ...
    public void buyItem(Player player, int itemTempId) {
        ...
        ItemShop ismabu = shop.getItemShop(578);
        Item ctdra = InventoryServiceNew.gI().findItemBody(player, 577);
        ...
        // if (shop.getItemShop(578) != null && ctdra == null) {
        // Service.gI().sendThongBao(player, "Vui lòng mua cải trang Drabula trước");
        // return;
        // }
        ...
        if (isCaitrangmabu(is) && !hasEnoughCaitrangDra(player)) {
            Service.gI().sendThongBao(player, "Vui lòng mua cải trang Drabula trước");
            return;
        }
        if (CaiTrangShop(is, 577) && !CanPhaiCoVatPham(player, 576, 1)) {
            Service.gI().sendThongBao(player, "Vui lòng mua cải trang Yacon trước");
            return;
        }
        if (CaiTrangShop(is, 576) && !CanPhaiCoVatPham(player, 575, 1)) {
            Service.gI().sendThongBao(player, "Vui lòng mua cải trang Bui Bui trước");
            return;
        }
        if (CaiTrangShop(is, 575) && !CanPhaiCoVatPham(player, 549, 1)) {
            Service.gI().sendThongBao(player, "Vui lòng mua cải trang Xên bọ hung con trước");
            return;
        }
        if (CaiTrangShop(is, 549) && !CanPhaiCoVatPham(player, 528, 1)) {
            Service.gI().sendThongBao(player, "Vui lòng mua cải trang Xên bọ hung 3 trước");
            return;
        }
        if (CaiTrangShop(is, 528) && !CanPhaiCoVatPham(player, 527, 1)) {
            Service.gI().sendThongBao(player, "Vui lòng mua cải trang Xên bọ hung 2 trước");
            return;
        }
        if (CaiTrangShop(is, 527) && !CanPhaiCoVatPham(player, 526, 1)) {
            Service.gI().sendThongBao(player, "Vui lòng mua cải trang Xên bọ hung 1 trước");
            return;
        }
    ...
    private boolean isCaitrangmabu(ItemShop itemShop) {
        int itemId = itemShop.temp.id;
        return itemId == 578;
    }

    private boolean CaiTrangShop(ItemShop itemShop, int idcaitrang) { // hàm để lấy vật phẩm muốn mua trong shop // By
                                                                      // Louis Nguyễn

        int itemId = itemShop.temp.id;
        return itemId == idcaitrang;
    }

    private boolean CanPhaiCoVatPham(Player player, int idvatpham, int soluong) { // hàm để check số lượng vật phầm cần
                                                                                  // để mới có thể mua // By Louis
                                                                                  // Nguyễn
        return player.inventory.itemsBag.stream()
                .filter(item -> item.isNotNullItem() && item.template.id == idvatpham && item.quantity >= soluong)
                .findFirst().isPresent();
    }
    ...
    private boolean hasEnoughCaitrangDra(Player player) {
        return player.inventory.itemsBag.stream()
                .filter(item -> item.isNotNullItem() && item.template.id == 577 && item.quantity != 0)
                .findFirst().isPresent();
    }
 ...
 * giả của mã nguồn này. Xin cảm ơn! - Louis Nguyễn
 */
