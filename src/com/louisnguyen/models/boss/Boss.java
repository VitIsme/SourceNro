...
import static com.louisnguyen.models.boss.BossStatus.ACTIVE;
import static com.louisnguyen.models.boss.BossStatus.CHAT_E;
import static com.louisnguyen.models.boss.BossStatus.CHAT_S;
import static com.louisnguyen.models.boss.BossStatus.DIE;
import static com.louisnguyen.models.boss.BossStatus.JOIN_MAP;
import static com.louisnguyen.models.boss.BossStatus.LEAVE_MAP;
import static com.louisnguyen.models.boss.BossStatus.RESPAWN;
import static com.louisnguyen.models.boss.BossStatus.REST;
import com.louisnguyen.models.boss.iboss.IBossNew;
import com.louisnguyen.models.boss.iboss.IBossOutfit;
...
public class Boss extends Player implements IBossNew, IBossOutfit {
    ...
    public Player getPlayerAttack() {
        if (this.playerTarger != null && (this.playerTarger.isDie() && !this.isNewPet  || !this.name.equals("Jajirô") || !this.zone.equals(this.playerTarger.zone))) {
...
//        System.out.println("this status: " + this.bossStatus + " (" + this.id + ")");
            ...
            if (this.currentLevel == 0) {
                if (this.parentBoss == null) {
                    ...
                    ChangeMapService.gI().changeMap(this, this.zone,
                            this.parentBoss.location.x + Util.nextInt(-100, 100), -1);
                }
    ...
    protected void notifyJoinMap() {
        ...
        if (this.zone.map.mapId == 140||MapService.gI().isMapMaBu(this.zone.map.mapId) ||MapService.gI().isMapDoanhTrai(this.zone.map.mapId) ||MapService.gI().isMapBanDoKhoBau(this.zone.map.mapId) ||MapService.gI().isMapGiaiCuuMiNuong(this.zone.map.mapId) || MapService.gI().isMapBlackBallWar(this.zone.map.mapId))
            return;
        ServerNotify.gI().notify("BOSS " + this.name + " vừa xuất hiện tại " + this.zone.map.mapName);
    }
    ...
    public void die(Player plKill) {
        if (plKill != null) {
            reward(plKill);
            ServerNotify.gI().notify(plKill.name + " vừa tiêu diệt được " + this.name + ", quá ngưởng mộ");
        }
...
//            ChangeMapService.gI().spaceShipArrive(this, (byte) 2, ChangeMapService.NON_SPACE_SHIP);
