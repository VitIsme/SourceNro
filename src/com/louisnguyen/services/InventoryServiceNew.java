...
//                    textGift += "x" + quantity + " " + itemGift.template.name + "\b";
    ...
    public void throwItem(Player player, int where, int index) {
        ...
        if (where == 0) {
            ...
            if (itemThrow.template.id != 530 && itemThrow.template.id != 531 && itemThrow.template.id != 534 && itemThrow.template.id != 535 && itemThrow.template.id != 536) {
    ...
    public void itemBodyToBag(Player player, int index) {
        ...
        if (item.isNotNullItem()) {
            if (index == 8) {
                Service.getInstance().sendPetFollow(player, 1, (short)(player.inventory.itemsBody.get(8).template.iconID + 1), (byte) 1, (short) 32, (short) 30, player.session, new byte[]{0, 1, 2});
            }
            if (index == 7) {
