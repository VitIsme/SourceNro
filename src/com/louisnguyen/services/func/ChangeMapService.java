...
public class ChangeMapService {
    ...
    public void openZoneUI(Player pl) {
        ...
        if (MapService.gI().isMapGiaiCuuMiNuong(pl.zone.map.mapId)) {
            Service.gI().sendThongBaoOK(pl, "Không thể đổi khu vực trong map này");
            return;
        }
        ...
        if (pl.zone.map.mapId >= 189 && pl.zone.map.mapId <= 191) {
            Service.gI().sendThongBaoOK(pl, "Không thể đổi khu vực trong map này");
            return;
        }
    ...
    public void changeZone(Player pl, int zoneId) {
        ...
        if (!pl.isAdmin() || !pl.isBoss) {
            ...
            if ((Gio >= 18 && Gio <= 20) && (pl.zone.map.mapId >= 68 && pl.zone.map.mapId <= 72) && (pl.playerTask.taskMain.id != 21)) {
                Service.gI().sendThongBaoOK(pl, "Không thể đổi khu vực vì đang trong thời gian hỗ trợ nhiệm vụ");
                return;
            }
            if ((Gio >= 18 && Gio <= 20) && (pl.zone.map.mapId >= 63 && pl.zone.map.mapId <= 67) && (pl.playerTask.taskMain.id != 21)) {
                Service.gI().sendThongBaoOK(pl, "Không thể đổi khu vực vì đang trong thời gian hỗ trợ nhiệm vụ");
                return;
            }
            if ((Gio >= 18 && Gio <= 20) && (pl.zone.map.mapId >= 73 && pl.zone.map.mapId <= 77) && (pl.playerTask.taskMain.id != 21)) {
                Service.gI().sendThongBaoOK(pl, "Không thể đổi khu vực vì đang trong thời gian hỗ trợ nhiệm vụ");
                return;
            }
            if ((Gio >= 18 && Gio <= 20) && (pl.zone.map.mapId >= 81 && pl.zone.map.mapId <= 83) && (pl.playerTask.taskMain.id != 22)) {
                Service.gI().sendThongBaoOK(pl, "Không thể đổi khu vực vì đang trong thời gian hỗ trợ nhiệm vụ");
                return;
            }
            if ((Gio >= 18 && Gio <= 20) && (pl.zone.map.mapId == 80) && (pl.playerTask.taskMain.id != 23)) {
                Service.gI().sendThongBaoOK(pl, "Không thể đổi khu vực vì đang trong thời gian hỗ trợ nhiệm vụ");
                return;
            }
        }
        if (pl.isAdmin() || pl.isBoss || Util.canDoWithTime(pl.iDMark.getLastTimeChangeZone(), 10000)) {
            ...
            Service.gI().sendThongBaoOK(pl, "Không thể đổi khu vực lúc này, vui lòng đợi "
                    + TimeUtil.getTimeLeft(pl.iDMark.getLastTimeChangeZone(), 15));
        }
    }
    ...
    public void changeMapInYard(Player pl, int mapId, int zoneId, int x) {
        ...
        if (zoneJoin != null) {
            ...
            changeMap(pl, zoneJoin, -1, -1, x, zoneJoin.map.yPhysicInTop(x, 100), TELEPORT_YARDRAT);
        }
    }
    ...
    private void changeMap(Player pl, Zone zoneJoin, int mapId, int zoneId, int x, int y, byte typeSpace) {
        ...
        if (zoneJoin != null) {
            ...
            boolean currMapIsTL = MapService.gI().isMapTL(pl.zone.map);
            boolean nextMapIsTL = MapService.gI().isMapTL(zoneJoin.map);
            ...
            if (pl.getSession() != null && pl.isAdmin()) {
//                Service.gI().sendEffPlayer(pl, pl, 57, 1, -1, 1);
            }
            ...
            if (currMapIsTL != nextMapIsTL) {
                Service.gI().sendThongBao(pl, "Bạn đã đến tương\nlai, khi chết ở đây\nbạn sẽ bị giảm sức\nmạnh và tiềm năng");
                Service.gI().point(pl);
                Service.gI().Send_Info_NV(pl);
            }
