...
public class CombineServiceNew {
    ...
    private static final int GOLD_BONG_TAI = 2_000_000_000;
    private static final int GOLD_KIEM_Z = 200_000_000;
    private static final int GEM_BONG_TAI = 10_000;
                ...
                if (player.combineNew.itemsCombine.size() == 2) {
                    ...
                    if (bongTai != null && manhVo != null && manhVo.quantity >= 9999) {
                        ...
                        this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                                "Cần Bông tai Porata và X9999 Mảnh vỡ bông tai", "Đóng");
                    }
                } else {
                    this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                            "Cần Bông tai Porata và X9999 Mảnh vỡ bông tai", "Đóng");
                ...
                if (InventoryServiceNew.gI().getCountEmptyBag(player) > 0) {
                    if (player.combineNew.itemsCombine.size() == 2) {
                        ...
                        if (player.combineNew.goldCombine <= player.inventory.gold) {
                            ...
                            Service.getInstance().sendThongBaoOK(player,
                                    "Cần 1 đá Pháp sư và 1 trang bị có thể Pháp sư");
                        }
                    } else {
                        Service.getInstance().sendThongBaoOK(player, "Không đủ hành trang");
                    }
            ...
            case MO_CHI_SO_BONG_TAI:
                if (player.combineNew.itemsCombine.size() == 3) {
                    ...
                    if (bongTai != null && manhHon != null && daXanhLam != null && manhHon.quantity >= 99) {
                        ...
                        npcSay += "Con cac dau buoi re rach" + "\n";
                        ...
                        this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                                "Cần Bông tai Porata cấp 2 và X99 Mảnh hồn bông tai và 1 Đá xanh lam", "Đóng");
                    }
                } else {
                    this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                            "Cần Bông tai Porata cấp 2 và X99 Mảnh hồn bông tai và 1 Đá xanh lam", "Đóng");
                        ...
                        if (star < MAX_STAR_ITEM) {
                            ...
                            if (player.combineNew.goldCombine <= player.inventory.gold) {
                                ...
                                baHatMit.createOtherMenu(player, ConstNpc.MENU_START_COMBINE, npcSay,
                                        "Nâng cấp\n" + (player.combineNew.gemCombine) + " ngọc\n"
                                                + "1 lần",
                                        "Nâng cấp\n" + (player.combineNew.gemCombine * 2) + " ngọc\n"
                                                + "10 Lần",
                                        "Nâng cấp\n" + (player.combineNew.gemCombine * 5) + " ngọc\n"
                                                + "100 lần");
                            } else {
                                npcSay += "Còn thiếu "
                                        + Util.numberToMoney(player.combineNew.goldCombine - player.inventory.gold)
        ...
        if (player.combineNew.itemsCombine.size() == 3) {
            ...
            if (linhthu != null && thangtinhthach != null && thangtinhthach.quantity >= 99) {
                ...
                if (Util.isTrue(player.combineNew.ratioCombine, 100)) {
                    ...
                    linhthu.itemOptions.add(new Item.ItemOption(38, 1));
                ...
                if (star < MAX_STAR_ITEM) {
                    ...
                    if (flag) {
                        ...
                        Service.getInstance().sendThongBao(player,
                                "Bạn đã đập " + (solandap - player.combineNew.quantities + 1) + " lần");
