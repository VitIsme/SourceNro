    ...
    private void useItem(Player pl, Item item, int indexBag) {
        ...
        if (item.template.strRequire <= pl.nPoint.power) {
            switch (item.template.type) {
                ...
                case 72: {
                    InventoryServiceNew.gI().itemBagToBody(pl, indexBag);
                    Service.getInstance().sendPetFollow(pl, 1, (short) (pl.inventory.itemsBody.get(8).template.iconID + 1), (byte) 1, (short) 32, (short) 30, pl.session, new byte[]{0, 1, 2});
                    break;
                }
                default:
                    switch (item.template.id) {
                        case 992:
                            if (calendar.get(Calendar.HOUR_OF_DAY) >= 14 && calendar.get(Calendar.HOUR_OF_DAY) <= 15) {
                                ...
                                Service.gI().sendThongBao(pl, "Hiện không thể tìm thấy tung tích của Kẻ phá hủy thời gian!!!");
                                break;
                            }
                        case 361:
                            if (pl.idNRNM != -1) {
...
//                            break;
                        case 1499:
                            ...
                            break;
                        case 1153: //Thần chết cute
                            int itemId = 1021;
                            Item PhanQua = ItemService.gI().createNewItem(((short) itemId));
                            PhanQua.itemOptions.add(new Item.ItemOption(50, Util.nextInt(5, 13)));
                            PhanQua.itemOptions.add(new Item.ItemOption(5, Util.nextInt(5, 10)));
                            PhanQua.itemOptions.add(new Item.ItemOption(77, Util.nextInt(1, 10)));
                            PhanQua.itemOptions.add(new Item.ItemOption(103, Util.nextInt(1, 10)));
                            PhanQua.itemOptions.add(new Item.ItemOption(93, Util.nextInt(5, 10)));

                            int itemId2 = 457;
                            Item PhanQua2 = ItemService.gI().createNewItem(((short) itemId2));
                            PhanQua2.quantity = 20;
...
//            if (player.pet.askPea() && player.pet.typePet == 1 && ((Pet) player).charms.tdMaBu2 > System.currentTimeMillis()) {
//                int statima = 100 * lvPea;
//                player.pet.nPoint.stamina += statima;
//                if (player.pet.nPoint.stamina > player.pet.nPoint.maxStamina) {
//                    player.pet.nPoint.stamina = player.pet.nPoint.maxStamina;
